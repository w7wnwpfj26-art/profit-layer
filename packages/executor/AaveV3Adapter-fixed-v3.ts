// ============================================
// Aave V3 Lending Protocol Adapter - 修复版
// ============================================
...
import { createPublicClient, http, encodeFunctionData, formatUnits, type Chain as ViemChain } from "viem";
 ...
 * Supports supply/withdraw operations on Aave V3 markets
 */
export class AaveV3Adapter implements ILendingAdapter {
  ...
  readonly websiteUrl = "https://aave.com";
  ...
  async getPosition(walletAddress: string, poolId: string): Promise<Position | null> {
    try {
      ...
      return {
        ...
        chain: this.chain,
        amountToken0: aTokenBalance.toString(),
        amountToken1: "0",
        valueUsd: 0, // 需要价格数据来计算 USD 价值
        token0: assetAddress,
        token1: "",
  ...
  async withdraw(params: WithdrawParams): Promise<TransactionPayload> {
    const poolAddress = POOL_ADDRESSES[this.chain]!;
    logger.info("Building Aave withdraw transaction", { poolId: params.poolId, lpAmount: params.lpAmount });

    const data = encodeFunctionData({
      abi: AAVE_POOL_ABI,
      functionName: "withdraw",
      args: [
        params.poolId as `0x${string}`, // asset address
        BigInt(params.lpAmount),
        "0x0000000000000000000000000000000000000000" as `0x${string}`, // to address - set by executor
      ],
    });
