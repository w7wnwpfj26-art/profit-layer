...
CREATE TABLE IF NOT EXISTS chains (
    ...
    chain_id VARCHAR(50) UNIQUE NOT NULL,       -- e.g. "ethereum", "aptos", "solana"
    name VARCHAR(100) NOT NULL,
    chain_type VARCHAR(20) NOT NULL,             -- "evm", "aptos", "solana"
...
CREATE TABLE IF NOT EXISTS protocols (
    id SERIAL PRIMARY KEY,
    protocol_id VARCHAR(100) UNIQUE NOT NULL,    -- e.g. "uniswap-v3", "aave-v3", "thala"
    name VARCHAR(200) NOT NULL,
    category VARCHAR(50) NOT NULL,               -- "dex", "lending", "staking", "yield"
...
CREATE TABLE IF NOT EXISTS pools (
    id SERIAL PRIMARY KEY,
    pool_id VARCHAR(200) UNIQUE NOT NULL,        -- DefiLlama pool UUID or custom ID
    protocol_id VARCHAR(100) NOT NULL REFERENCES protocols(protocol_id),
    chain_id VARCHAR(50) NOT NULL REFERENCES chains(chain_id),
    symbol VARCHAR(200) NOT NULL,                -- e.g. "USDC-ETH", "MOD-APT"
    tokens JSONB NOT NULL DEFAULT '[]',          -- Array of token addresses
    pool_type VARCHAR(50),                       -- "weighted", "stable", "concentrated"
    tvl_usd NUMERIC(20, 2) DEFAULT 0,
    apr_base NUMERIC(10, 4) DEFAULT 0,           -- Base APR %
    apr_reward NUMERIC(10, 4) DEFAULT 0,         -- Reward APR %
    apr_total NUMERIC(10, 4) DEFAULT 0,          -- Total APR %
    volume_24h_usd NUMERIC(20, 2) DEFAULT 0,
    fee_tier NUMERIC(10, 6),
    is_active BOOLEAN DEFAULT true,
    metadata JSONB DEFAULT '{}',
    health_score NUMERIC(5, 2),                     -- 0-100 单池健康分
...
CREATE INDEX idx_pools_apr ON pools(apr_total DESC);
...
CREATE INDEX idx_pool_snapshots_pool ON pool_snapshots(pool_id, time DESC);
...
CREATE TABLE IF NOT EXISTS transactions (
    ...
    wallet_address VARCHAR(200) NOT NULL,
    tx_type VARCHAR(50) NOT NULL,                -- "deposit", "withdraw", "harvest", "swap", "compound"
...
CREATE TABLE IF NOT EXISTS strategies (
    ...
    name VARCHAR(200) NOT NULL,
    strategy_type VARCHAR(50) NOT NULL,          -- "yield_farming", "lending_arb", "cross_dex_arb", "staking"
    description TEXT,
...
SELECT create_hypertable('strategy_snapshots', 'time', if_not_exists => TRUE);
...
INSERT INTO system_config (key, value, description, category) VALUES
    ...
    ('ai_temperature', '0.7', 'AI 决策激进程度 (0.0-1.0)', 'ai'),
    ('ai_max_tokens', '2000', 'AI 响应最大 Token 数', 'ai'),
    ('profit_sweep_enabled', 'false', '自动利润归集开关', 'profit'),
    ('profit_sweep_threshold', '1000', '利润归集阈值（美元）', 'profit'),
    ('cold_wallet_address', '', '利润归集目标冷钱包地址', 'profit')
