# DeFi 數據庫啟動與管理指南

## 📋 概述

本項目使用 **TimescaleDB**（基於 PostgreSQL）作為主數據庫，用於存儲：
- DeFi 池子數據（pools、protocols）
- 用戶持倉信息（positions）
- 交易記錄（transactions）
- 時序數據（pool_snapshots、strategy_snapshots）

## 🚀 快速開始

### 1. 啟動數據庫（推薦方式）

使用提供的腳本一鍵啟動：

```bash
# 僅啟動數據庫
bash scripts/start-database.sh

# 啟動數據庫 + Redis + Grafana
bash scripts/start-database.sh --all

# 啟動並查看日誌
bash scripts/start-database.sh --logs
```

### 2. 驗證數據庫

```bash
# 檢查表結構和數據
bash scripts/check-database.sh
```

### 3. 啟動 Dashboard

```bash
cd packages/dashboard
pnpm install  # 首次運行需要
pnpm dev
```

然後訪問：http://localhost:3002

---

## 📊 數據庫配置

### 默認配置

| 配置項 | 默認值 | 說明 |
|--------|--------|------|
| 主機 | `localhost` | 本地開發使用 |
| 端口 | `5433` | 避免與本機 PostgreSQL 衝突 |
| 數據庫名 | `defi_yield` | 數據庫名稱 |
| 用戶名 | `defi` | 數據庫用戶 |
| 密碼 | `change_me_in_production` | ⚠️ 生產環境請修改 |

### 修改配置

編輯 `.env` 文件：

```bash
# 數據庫配置
POSTGRES_HOST=localhost
POSTGRES_PORT=5433
POSTGRES_DB=defi_yield
POSTGRES_USER=defi
POSTGRES_PASSWORD=your_secure_password_here
```

---

## 🛠️ 常用命令

### Docker Compose 命令

```bash
# 啟動數據庫
docker compose up -d timescaledb

# 停止數據庫
docker compose stop timescaledb

# 重啟數據庫
docker compose restart timescaledb

# 查看日誌
docker compose logs -f timescaledb

# 查看容器狀態
docker compose ps

# 停止並刪除容器（數據保留）
docker compose down

# 停止並刪除容器和數據卷（⚠️ 危險）
docker compose down -v
```

### 數據庫命令行

```bash
# 進入 PostgreSQL 命令行
docker compose exec timescaledb psql -U defi -d defi_yield

# 在命令行中可以執行：
\dt              # 列出所有表
\d pools         # 查看 pools 表結構
\q               # 退出

# 執行 SQL 查詢（不進入命令行）
docker compose exec -T timescaledb psql -U defi -d defi_yield -c "SELECT COUNT(*) FROM pools;"
```

---

## 📁 數據庫表結構

### 主要表

| 表名 | 說明 | 類型 |
|------|------|------|
| `chains` | 支持的區塊鏈 | 基礎表 |
| `protocols` | DeFi 協議 | 基礎表 |
| `pools` | 流動性池 | 數據表 |
| `pool_snapshots` | 池子歷史快照 | 時序表 |
| `positions` | 用戶持倉 | 數據表 |
| `transactions` | 交易記錄 | 數據表 |
| `strategies` | 策略配置 | 基礎表 |
| `strategy_snapshots` | 策略歷史快照 | 時序表 |
| `system_config` | 系統配置 | 配置表 |
| `audit_log` | 審計日誌 | 日誌表 |

### 查看表數據

```bash
# 查看鏈列表
docker compose exec -T timescaledb psql -U defi -d defi_yield -c "SELECT * FROM chains;"

# 查看池子數量
docker compose exec -T timescaledb psql -U defi -d defi_yield -c "SELECT COUNT(*) FROM pools;"

# 查看 APR 最高的 10 個池子
docker compose exec -T timescaledb psql -U defi -d defi_yield -c "
SELECT pool_id, protocol_id, chain_id, symbol, apr_total, tvl_usd 
FROM pools 
WHERE is_active = true 
ORDER BY apr_total DESC 
LIMIT 10;"
```

---

## 🔧 故障排查

### 問題 1：Dashboard 連不上數據庫

**症狀：**
```
Error: connect ECONNREFUSED 127.0.0.1:5433
```

**解決方案：**

1. 確認數據庫運行：
   ```bash
   docker compose ps timescaledb
   # 應該顯示 "Up" 狀態
   ```

2. 檢查端口映射：
   ```bash
   docker compose port timescaledb 5432
   # 應該顯示: 0.0.0.0:5433
   ```

3. 驗證連接：
   ```bash
   bash scripts/check-database.sh
   ```

4. 檢查 `.env` 配置：
   ```bash
   grep POSTGRES_ .env
   ```

### 問題 2：數據庫啟動失敗

**可能原因：**
- 端口被佔用
- Docker 資源不足
- 數據卷損壞

**解決方案：**

```bash
# 1. 查看詳細日誌
docker compose logs timescaledb

# 2. 檢查端口佔用
lsof -i :5433  # macOS/Linux
netstat -ano | findstr :5433  # Windows

# 3. 重新創建容器
docker compose down
docker compose up -d timescaledb

# 4. 如果數據卷損壞（⚠️ 會清空數據）
docker compose down -v
docker compose up -d timescaledb
```

### 問題 3：數據庫數據為空

**檢查步驟：**

```bash
# 1. 確認表已創建
bash scripts/check-database.sh

# 2. 查看初始化日誌
docker compose logs timescaledb | grep -i "init"

# 3. 手動執行初始化腳本
docker compose exec -T timescaledb psql -U defi -d defi_yield < infra/postgres/init.sql
```

如果 `pools` 和 `protocols` 表為空，需要：
- 運行 Scanner 服務來抓取實時數據
- 或運行種子腳本填充模擬數據：`pnpm seed`

---

## 🔐 安全建議

### 開發環境

- ✅ 使用默認配置即可
- ✅ 端口 5433 僅監聽 localhost

### 生產環境

1. **修改密碼**
   ```bash
   # .env
   POSTGRES_PASSWORD=使用強密碼
   ```

2. **限制訪問**
   ```yaml
   # docker-compose.yml
   services:
     timescaledb:
       ports:
         - "127.0.0.1:5433:5432"  # 僅本地訪問
   ```

3. **備份數據**
   ```bash
   # 備份
   docker compose exec -T timescaledb pg_dump -U defi defi_yield > backup.sql
   
   # 恢復
   docker compose exec -T timescaledb psql -U defi -d defi_yield < backup.sql
   ```

4. **啟用 SSL**（可選）
   - 配置 PostgreSQL SSL 證書
   - 修改連接字符串添加 `sslmode=require`

---

## 📈 性能優化

### 1. 連接池配置

Dashboard 已配置連接池（最大 5 個連接）：

```typescript
// packages/dashboard/app/api/pools/route.ts
const pool = new pg.Pool({
  max: 5,  // 最大連接數
  // ...
});
```

### 2. 索引優化

數據庫已創建必要索引：

```sql
-- 查看現有索引
SELECT tablename, indexname, indexdef 
FROM pg_indexes 
WHERE schemaname = 'public';
```

### 3. 時序數據清理

定期清理舊的快照數據：

```sql
-- 刪除 90 天前的池子快照
DELETE FROM pool_snapshots 
WHERE time < NOW() - INTERVAL '90 days';

-- 刪除 180 天前的策略快照
DELETE FROM strategy_snapshots 
WHERE time < NOW() - INTERVAL '180 days';
```

---

## 🔄 數據遷移

### 導出數據

```bash
# 導出整個數據庫
docker compose exec -T timescaledb pg_dump -U defi defi_yield > defi_yield_backup.sql

# 僅導出表結構
docker compose exec -T timescaledb pg_dump -U defi -s defi_yield > schema.sql

# 僅導出數據
docker compose exec -T timescaledb pg_dump -U defi -a defi_yield > data.sql
```

### 導入數據

```bash
# 導入整個備份
docker compose exec -T timescaledb psql -U defi -d defi_yield < defi_yield_backup.sql

# 或使用管道
cat defi_yield_backup.sql | docker compose exec -T timescaledb psql -U defi -d defi_yield
```

---

## 🔗 相關資源

### 項目文檔
- [Git 與更新配置說明](./GIT与更新配置说明.md)
- [桌面應用發布流程](../packages/desktop/RELEASE.md)
- [項目主 README](../README.md)

### 官方文檔
- [TimescaleDB 文檔](https://docs.timescale.com/)
- [PostgreSQL 文檔](https://www.postgresql.org/docs/)
- [Docker Compose 文檔](https://docs.docker.com/compose/)

### 常用工具
- [pgAdmin](https://www.pgadmin.org/) - PostgreSQL 圖形化管理工具
- [DBeaver](https://dbeaver.io/) - 通用數據庫管理工具
- [TablePlus](https://tableplus.com/) - macOS 數據庫客戶端

---

## 📞 獲取幫助

如果遇到問題：

1. 查看日誌：`docker compose logs -f timescaledb`
2. 運行檢查腳本：`bash scripts/check-database.sh`
3. 查看本文檔的故障排查章節
4. 檢查 Docker 和磁盤空間是否充足

---

**最後更新：** 2026-02-07
